...
def setup_page():
    st.set_page_config(
        page_title="The Game: Review Hunters 🕵️‍♂️ – Stage 1",
        layout="wide",
...
def initialize_session_state(df):    
    if "stage_1_current_review" not in st.session_state:
        st.session_state.stage_1_current_review = 0

    if "stage_1_answers" not in st.session_state:
        st.session_state.stage_1_answers = [{} for _ in range(len(df))]

    # Tracking time
    if "stage_1_start_time" not in st.session_state:
        st.session_state.stage_1_start_time = datetime.now()
    if "stage_1_end_time" not in st.session_state:
        st.session_state.stage_1_end_time = None
...
def render_questions(current: int, saved: dict):
    ...
    with col_center:
        ...
        raw_decision = st.radio(
            ...
            index=preselected,           # ← changed logic
            horizontal=True,
...
def get_response(
    ...
    Returns a dict with the fields we care about for pre-filling Stage 1,
    ...
    # Map the fields we actually use in Stage 1
    saved = {}
...
def handle_navigation(current: int, total: int, answers: dict, review_id: int, hub):
    ...
    with col1:
        if current > 0:
            if st.button("← Previous", width="stretch"):
                st.session_state.stage_1_current_review -= 1
    ...
    with col2:
        ...
        if st.button(label, width="stretch"):
            ...
            st.session_state.stage_1_answers[current] = answers.copy()
            save_response(review_id, answers, hub)
            # Tracking End time for stage 1
            if current == total - 1:
                if st.session_state.stage_1_end_time is None:
                    st.session_state.stage_1_end_time = datetime.now()
...
def main():
    ...
    setup_page()
    stage_1_df = filter_data(df=st.session_state.reviews_df, stage=1)
    initialize_session_state(stage_1_df)
    # Reset the scrolling
    scroll_to_top()
    ...
    st.markdown("<h1>Stage 1: Solo Sleuthing 🔍</h1>", unsafe_allow_html=True)

    total = len(stage_1_df)
    current = st.session_state.stage_1_current_review
    ...
    saved_answers = saved_from_db or st.session_state.stage_1_answers[current]
